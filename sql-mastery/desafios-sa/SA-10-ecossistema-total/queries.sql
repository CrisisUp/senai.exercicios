...
CREATE TABLE IF NOT EXISTS historico_operacoes (
    ...
    drone_id INTEGER,
    distancia REAL,
    custo_operacional_centavos INTEGER NOT NULL DEFAULT 0, -- Guardião Financeiro
...
CREATE TRIGGER IF NOT EXISTS trg_proteger_em_missao 
BEFORE UPDATE ON drones
WHEN OLD.status = 'EM_MISSAO' 
BEGIN
    SELECT RAISE(
        ABORT,
...
SELECT d.serial,
    d.modelo,
    h.distancia,
    printf('R$ %.2f', h.custo_operacional_centavos / 100.0) AS "Custo Operacional", -- Formatação Financeira
    ...
    RANK() OVER(ORDER BY h.distancia DESC) AS "Rank Global Distância"
FROM drones d
...
INSERT INTO historico_operacoes (drone_id, distancia, custo_operacional_centavos, telemetria_json)
VALUES 
    (1, 45.5, 15075, '{"bateria": 88, "sat": 12}'), -- R$ 150,75
    (1, 50.2, 18500, '{"bateria": 75, "sat": 10}'), -- R$ 185,00
    (2, 120.0, 45000, '{"bateria": 45, "sat": 8}'); -- R$ 450,00
