...
void GerenciadorAtendimento::carregarPacientes(const string& caminhoArquivo) {
    ...
    cout << "\n" << UI::CIANO << "[SISTEMA]: " << UI::RESET << "Carregando lista de espera de: " << caminhoArquivo << "..." << endl;
    ...
    if (arquivo.is_open()) {
        ...
        arquivo.close();
        cout << UI::VERDE << "[SUCESSO]: " << UI::RESET << "Lista carregada com sucesso!" << endl;
    } else {
...
void GerenciadorAtendimento::adicionarPaciente(const string& nome) {
    filaAtendimento.push(nome);
    cout << UI::VERDE << "[OK]: " << UI::RESET << nome << " entrou no final da fila." << endl;
}
