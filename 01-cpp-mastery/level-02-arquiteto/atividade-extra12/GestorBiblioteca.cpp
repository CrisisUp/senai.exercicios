...
void GestorBiblioteca::carregarCatalogo(const string& caminho) {
    ...
    arquivo.close();
    cout << UI::CIANO << "[SISTEMA]: " << UI::RESET << catalogo.size() << " títulos carregados." << endl;
}

void GestorBiblioteca::adicionarLeitor(const string& nome) {
    filaLeitores.push(nome);
    cout << UI::VERDE << "[OK]: " << UI::RESET << nome << " entrou na fila." << endl;
}
...
void GestorBiblioteca::realizarEmprestimo(int index) {
    ...
    if (index >= 0 && index < (int)catalogo.size()) {
        ...
        cout << UI::VERDE << "[SUCESSO]: " << UI::RESET << "Empréstimo de '" << emp.livro.titulo << "' registrado!" << endl;
    } else {
...
void GestorBiblioteca::desfazerEmprestimo() {
    ...
    cout << UI::AMARELO << "[DESFEITO]: " << UI::RESET << "Livro '" << ultimo.livro.titulo << "' voltou ao estoque." << endl;
    historico.pop();
}
...
void GestorBiblioteca::salvarRelatorio(const string& caminho) {
    ...
    rel.close();
    cout << UI::CIANO << "[SISTEMA]: " << UI::RESET << "Relatório gerado com segurança em " << caminho << endl;
}
