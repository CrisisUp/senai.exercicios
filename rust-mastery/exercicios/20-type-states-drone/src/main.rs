...
impl Drone<Manutencao> {
    fn new(id: &str) -> Self {
        println!("\x1b[33m[SISTEMA]: Drone {} criado em estado de MANUTENÇÃO.\x1b[0m", id);
        Self {
            id: id.to_string(),
            state: PhantomData,
        }
    ...
    fn preparar_para_voo(self) -> Drone<Terra> {
        println!("\x1b[32m[OK]: Calibragem concluída. Drone {} pronto na pista.\x1b[0m", self.id);
        Drone {
            id: self.id,
            state: PhantomData,
        }
...
impl Drone<Terra> {
    ...
    fn decolar(self) -> Drone<Voo> {
        println!("\x1b[35m[VOO]: Drone {} decolando! Motores em potência máxima.\x1b[0m", self.id);
        Drone {
            id: self.id,
            state: PhantomData,
        }
    ...
    fn recolher_para_oficina(self) -> Drone<Manutencao> {
        println!("\x1b[33m[OFFLINE]: Drone {} enviado para manutenção preventiva.\x1b[0m", self.id);
        Drone {
            id: self.id,
            state: PhantomData,
        }
...
impl Drone<Voo> {
    ...
    fn pousar(self) -> Drone<Terra> {
        println!("\x1b[32m[POUSO]: Drone {} pousou com segurança.\x1b[0m", self.id);
        Drone {
            id: self.id,
            state: PhantomData,
        }
    ...
    fn realizar_missao(&self) {
        println!("   ... Drone {} processando dados de imagem a 500 pés ...", self.id);
    }
}
