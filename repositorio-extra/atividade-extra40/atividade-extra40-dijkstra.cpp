...
private:
    ...
    void renderizarResultado(int ori, int dest, const vector<int>& dists, const vector<int>& pais) {
        ...
        cout << UI::BRANCO << "Hospital  : " << UI::RESET << catalogoLocais[ori] << endl;
        cout << UI::BRANCO << "Emergência: " << UI::RESET << catalogoLocais[dest] << endl;
        cout << UI::BRANCO << "ETA (Tempo): " << UI::RESET << UI::VERDE << UI::NEGRITO << dists[dest] << " min" << UI::RESET << endl;
