...
int main() {
    ...
    cout << UI::BRANCO << "Status de Boot: " << UI::RESET << turbina.statusMotor() << endl;
    ...
    while (turbina.estaLigado()) {
        cout << "\n" << UI::BRANCO << "Monitoramento Ativo. Informe Pressão (psi): " << UI::RESET;
        ...
        if (turbina.atualizarOperacao(leituraPSI)) {
            ...
            cout << UI::BRANCO << " >> " << UI::RESET << turbina.statusMotor() << endl;
        } else {
